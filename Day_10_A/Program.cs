...
";
            ...
            var vectors = input.Replace("position=<", "").Replace("> velocity=<", ";").Split(">", StringSplitOptions.RemoveEmptyEntries)
                .Where(x => !string.IsNullOrWhiteSpace(x)).Select(x => x.Trim())
                .Select(x => x.Split(";", StringSplitOptions.RemoveEmptyEntries))
                .Select(x => new { position = x[0].Split(",", StringSplitOptions.RemoveEmptyEntries), speed = x[1].Split(",", StringSplitOptions.RemoveEmptyEntries) })
                .Select(x => new Vector( long.Parse(x.position[0]), long.Parse(x.position[1]), long.Parse(x.speed[0]), long.Parse(x.speed[1]) )).ToList();

            var vectorSpace = new VectorSpace(vectors);

            long previousArea = 0;
            var iteration = 0;
            var currentArea = vectorSpace.MaxArea;
            do
            {
                previousArea = currentArea;
                vectorSpace.Move(1);
                currentArea = vectorSpace.MaxArea;
                iteration++;
            } while (currentArea <= previousArea);

            vectorSpace.Move(-1);
            foreach (var vector in vectorSpace.Vectors)
            {
                Console.SetCursorPosition((int)vector.X - (int)vectorSpace.Left, (int)vector.Y - (int)vectorSpace.Top);
                Console.Write("X");
            }
            Console.WriteLine();
        ...
        {
            private IEnumerable<Vector> vectors;

            public VectorSpace(IEnumerable<Vector> vectors)
            {
                this.vectors = vectors ?? new List<Vector>();
            }

            public IEnumerable<Vector> Vectors => vectors;

            public void Move(long timeSeconds)
            {
                vectors = Vectors.Select(x=> x.Move(timeSeconds));
            }
            ...
            public long Buttom
            {
                get
                {
                    return Vectors.Max(x => x.Y);
                }
            }
        }
        public class Vector
        {
            public Vector(long x, long y, long speedX, long speedY)
            {
                X = x;
                Y = y;
                SpeedX = speedX;
            ...
            public long X { get; }
            public long Y { get; }
            public long SpeedX { get; }
            public long SpeedY { get; }

            public Vector Move(long timeSeconds)
            {
                return new Vector(X + SpeedX * timeSeconds, Y + SpeedY * timeSeconds , SpeedX, SpeedY);
            }
        }
